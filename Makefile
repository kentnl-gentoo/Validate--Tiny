# This Makefile is for the Validate::Tiny extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Validate/Tiny.pm]
#     AUTHOR => [q[minimalist <minimalist@lavabit.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     MIN_PERL_VERSION => q[5.008]
#     NAME => q[Validate::Tiny]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0.94] }
#     VERSION_FROM => q[lib/Validate/Tiny.pm]
#     clean => { FILES=>q[Validate-Tiny-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Validate/Tiny.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Validate
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Validate
...
TO_INST_PM = lib/Validate/Tiny.pm

PM_TO_BLIB = lib/Validate/Tiny.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Validate/Tiny.pm $(INST_MAN3DIR)/Validate::Tiny.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Minimalistic data validation'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''minimalist <minimalist@lavabit.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Validate-Tiny' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Minimalistic data validation",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "minimalist <minimalist@lavabit.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Validate-Tiny",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Minimalistic data validation</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>minimalist &lt;minimalist@lavabit.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Validate/Tiny.pm blib/lib/Validate/Tiny.pm 
